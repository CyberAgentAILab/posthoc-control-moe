...
def main_worker(ngpus_per_node, args):
    ...
    if args.evaluate:
        ...
        _acc = validate(
            val_loader, model, accelerator, -1, False, args, datasetname=args.dataset
        )
        _acc = validate(
            test_loader, model, accelerator, -1, True, args, datasetname=args.dataset
        )
        if args.dataset == "fever":
            _acc = validate(
                test_loader_v2,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        val_acc = validate(
            val_loader, model, accelerator, epoch, False, args, datasetname=args.dataset
        )
    ...
    logger.info("Best Epoch: {}, Best val Acc: {:.3f}".format(best_epoch, best_val))
